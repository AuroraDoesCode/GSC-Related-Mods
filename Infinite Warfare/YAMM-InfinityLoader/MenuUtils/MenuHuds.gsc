...
*    Description : Menu Huds for YetAnotherModMenu
...
drawText()
{
    ...
    {
        self.menu["OPT"]["TITLE"] = self createText("objective",1.3,"CENTER","CENTER",0,-232,10,0,"Physics N Flex V3",(1, 1, 1),true);

        self.menu["OPT"]["TITLE"] affectElement("alpha", .4, 1);

        self.menu["OPT"]["SUB_TITLE"] = self createText("objective",1.2,"LEFT","CENTER",-118,-210,10,0,self.menuTitle,(1, 1, 1),true);

        self.menu["OPT"]["SUB_TITLE"] affectElement("alpha", .4, 1);

        self.menu["OPT"]["OPTSize"] = self createText("objective",1.2,"CENTER","CENTER",110,-210,10,0,self getCursor() + 1 + "/" + self.eMenu.size,(1, 1, 1),true);

        self.menu["OPT"]["OPTSize"] affectElement("alpha", .4, 1);

        for(e = 0; e < 8; e++)
        {
            self.menu["OPT"][e] = self createText("objective",1.2,"LEFT","CENTER",-121,-195 + e * 12,4,0,"",(1, 1, 1),true);
